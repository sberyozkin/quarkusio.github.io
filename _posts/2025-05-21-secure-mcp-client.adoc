...
public class SecurityIdentityPermissionAugmentor implements SecurityIdentityAugmentor { <1>
    ...
    static class HibernateBlockingAugmentor {
        ...
        public SecurityIdentity augment(SecurityIdentity securityIdentity) {
            QuarkusSecurityIdentity.Builder builder = QuarkusSecurityIdentity.builder(securityIdentity); <3>
            UserInfo userInfo = securityIdentity.getAttribute(OidcUtils.USER_INFO_ATTRIBUTE); <4>
            Identity identity = Identity.findByName(userInfo.getName()); <5>
            builder.addPermissionAsString(identity.permission); <6>
...
<3> Initialize a security identity builder from the current identity.
<4> Get GitHub specific `UserInfo` representation stored as the security identity attribute.
<5> Find the record matching the current user's name.
<6> Add the permission allocated to this user to the security identity.
<7> Create an updated `SecurityIdentity`.
